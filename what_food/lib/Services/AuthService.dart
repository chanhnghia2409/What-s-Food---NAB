...
class AuthService {
  ...
  http.Response response = await http.post(apiUrl, body:  {'phone':phone,'password': password, 'email': email, 'name': name});
  ...
  static Future <User> getUserWithId() async {
    ...
    http.Response response = await http.get(apiUrl, headers: {'Authorization' : 'Bearer $token'});
    if (response.statusCode == 200) {
        print("Result: ${response.body}");
  ...
  static Future logout_Author() async {
    ...
    http.Response response = await http.post(apiUrl, headers: {'Authorization' : 'Bearer $token'});
    if (response.statusCode == 200) {
